...
export default function InterviewQuestionsPage() {
  ...
  const loadCategories = () => {
    ...
    allQuestions.forEach(q => {
      if (!categoryStats[q.category]) {
        categoryStats[q.category] = { count: 0, highFreq: 0 };
      }
      categoryStats[q.category].count++;
      if (q.frequency >= 4) {
        categoryStats[q.category].highFreq++;
      }
    });
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto">
      {/* 页面标题 */}
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900 mb-2">面试题库</h1>
        <p className="text-gray-600">按技术领域分类的面试题目，帮助你针对性准备</p>
      </div>
      ...
      {!selectedCategory && (
        <div className="space-y-6">
          {categories.map((category) => (
            <div
              key={category.id}
              onClick={() => loadCategoryQuestions(category.id)}
            ...
            >
              ...
              {category.highFreqCount > 0 && (
                <div className="text-xs text-red-600">
                  包含 {category.highFreqCount} 个高频题目
                </div>
              )}
            </div>
          ))}
