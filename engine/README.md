...
一个本地优先、直接读取 Obsidian 风格 Markdown Vault 的个人 AI 工作台。本目录只保存应用代码；示例知识库位于旁边的 `../kb/`。

Workbench 提供可复用的读取、索引和可视化能力，不包含任何作者的个人知识、账号数据、登录态或历史运行记录。仓库级 `kb/` 是完全虚构的演示 Vault，用于展示素材阅读、Wiki、知识星图、灵感、内容与抖音数据页面。
...
- 可运行演示：`../kb/30_self_media/douyin/current.json`
...
扫描不会代替人工审核。还需要检查 Git 历史、图片、录屏、测试夹具、构建产物和本地配置。详细清单见 [公开边界说明](../docs/workbench/public-release-boundaries.md)。
